...
- The search decider (`search_decider_model`) only ever sees a bounded recent slice of the conversation (roughly 8k tokens, reserving room for its fixed prompt). It decides "should we search?" from the latest query plus recent context, so near the context window it does not carry — and then auto-compact — the entire conversation, which would otherwise cost hundreds of model calls before any reply could stream. Main-model replies and the decider both compact independently, each capped at their own window.
