...
- The search decider (`search_decider_model`) runs the exact same conversation pipeline as the main model: the reply chain is walked and augmented (video URLs, document extraction, media analysis, visual search, website/youtube/reddit content) using the decider model's own content options. The only difference is that the search decider prompt is always prepended to the latest user query in the decider's request.
