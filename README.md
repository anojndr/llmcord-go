...
- Context-window enforcement using each model's `context_window`: requests that would exceed the window are rejected with a clear "context window limit reached" error instead of being auto-compacted
...
| `context_window` | Optional per-provider context windows (plain ints or `k`/`m` suffixes), applied to models without their own value. Used to enforce the context-window limit: requests that would exceed it are rejected with a "context window limit reached" error. See model notes. |
...
- `context_window` is local metadata for retained-context reply-footers and the context-window limit check. Provider-only tokens (hidden reasoning) aren't counted; text is token-estimated with Codex's `ceil(bytes/4)` ratio. It also derives the per-message character limit: one message is capped at roughly one window of text, so oversized pastes and text attachments are truncated to fit. Without a configured window there is no per-message cap and no limit check. Requests whose estimated tokens reach the configured window are rejected with a "that query would exceed the model context window" error — history is never auto-compacted or truncated to fit.
- The search decider (`search_decider_model`) runs the exact same conversation pipeline as the main model: the reply chain is walked and augmented (video URLs, document extraction, media analysis, visual search, website/youtube/reddit content) using the decider model's own content options, and the request is checked against the decider model's own context window. The only difference is that the search decider prompt is always prepended to the latest user query in the decider's request.
