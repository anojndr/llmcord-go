...
| `models` | Ordered list of `<provider>/<model>` entries. The first entry is the startup default. Append `:vision` for image support heuristics. Gemini suffix aliases like `-minimal`, `-low`, `-medium`, and `-high` control thinking level. Codex suffix aliases like `-none`, `-minimal`, `-low`, `-medium`, `-high`, and `-xhigh` control reasoning effort. |
...
- If a provider has multiple API keys, the bot tries them in order until one succeeds or all fail. Gemini rate-limit responses that include a retry delay wait on the same key once before rotating to the next key.
