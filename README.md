...
- `View on Rentry` button on final replies that publishes the assistant response to Rentry on demand for easier reading, plus a `Show Sources` button on web-search and visual-search replies that opens a paginated ephemeral view of the web queries, parsed web source URLs, and visual-search result URLs used, and a `Show Thinking` button on final replies that had streamed reasoning so users can reopen the persisted thinking in an ephemeral paginated view after the visible answer collapses back to answer-only
...
- In embed streaming mode, Gemini thought summaries and OpenAI Codex reasoning summaries are rendered live above the answer text while generation is in progress. The final reply collapses back to answer-only, but when a reply streamed thinking it also keeps a `Show Thinking` button that reopens the persisted thinking in an ephemeral paginated view.
- When `database.connection_string` is configured, persisted assistant history also stores the streamed thinking separately, so the `Show Thinking` button still works after bot restarts for retained reply-chain messages.
