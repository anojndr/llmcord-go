...
func assertDefaultLoadedConfig(t *testing.T, loadedConfig config) {
	...
	if !loadedConfig.Providers["openai"].ChainPreviousResponse {
		t.Fatal("expected chain_previous_response to default to true")
	}
}
...
`
	...
	if providers.ProviderAPIKind(loadedConfig.Providers["gemini"].apiKind()) != providers.ProviderAPIKindGemini {
...
func TestLoadConfigChainsPreviousResponsePerProvider(t *testing.T) {
	t.Parallel()
...
`
	...
	if loadedConfig.Providers["openai"].ChainPreviousResponse {
		t.Fatal("expected chain_previous_response to be false for openai")
	}

	if !loadedConfig.Providers["gemini"].ChainPreviousResponse {
		t.Fatal("expected chain_previous_response to default to true for gemini")
	}
}

func TestLoadConfigExaSearchTypePerProvider(t *testing.T) {
	t.Parallel()
...
`
	...
	if loadedConfig.Providers["openai"].ExaSearchType != exaSearchTypeDeep {
		t.Fatalf("unexpected provider exa search type: %q", loadedConfig.Providers["openai"].ExaSearchType)
	}

	if loadedConfig.Providers["gemini"].ExaSearchType != "" {
		t.Fatalf("expected empty provider exa search type default, got %q", loadedConfig.Providers["gemini"].ExaSearchType)
	}
}

func TestLoadConfigRejectsInvalidProviderExaSearchType(t *testing.T) {
	t.Parallel()
...
`
	...
	_, err = loadConfig(configPath)
	if err == nil {
		t.Fatal("expected invalid provider exa_search_type to fail validation")
	}
}
...
`
	...
	if !found {
		t.Fatal("missing model parameters")
	}
...
func TestLoadConfigIgnoresLegacyExaLivecrawlTimeout(t *testing.T) {
	t.Parallel()
...
`
	...
	// The knob is retained in raw config only so existing user configurations
	// keep loading; Exa requests are cache-only and never read it.
	if _, err := loadConfig(configPath); err != nil {
		t.Fatalf("load config with legacy livecrawl timeout: %v", err)
	}
}
