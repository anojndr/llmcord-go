...
func (client serpAPIGoogleLensClient) searchOnce(
	...
	if httpResponse.StatusCode < http.StatusOK || httpResponse.StatusCode >= http.StatusMultipleChoices {
		return emptyVisualSearchResult(), providers.NewSerpAPIProviderError(
...
func (client serpAPIGoogleLensClient) parseResponse(
	...
	case status == "":
		if responseError != "" {
			return emptyVisualSearchResult(), providers.StatusError{
	...
	case strings.EqualFold(status, providers.SerpAPISearchStatusSuccess):
		return parseSerpAPIGoogleLensResponse(imageURL, response), nil
	default:
		return emptyVisualSearchResult(), providers.NewSerpAPISearchStatusError(
			imageURL,
			status,
			responseError,
		)
