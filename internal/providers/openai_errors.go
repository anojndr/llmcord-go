...
func openAIHTTPErrorBodyMessage(responseBody []byte) string {
	...
	if support.RuneCount(bodyText) > openAIHTTPErrorBodyPreviewRunes {
		return ""
	}
...
// OpenAIHTTPErrorBodyLooksOpaque reports whether an error body is unusable.
func OpenAIHTTPErrorBodyLooksOpaque(bodyText string) bool {
	...
	for _, field := range fields {
		fieldRunes := support.RuneCount(field)
...
// NewOpenAIProviderStatusError builds a provider status error with friendly text.
