...
- Message event: `bot.handleMessageCreate` (`internal/app/messages.go`, wrapped in `recoverHandler`) → 30s/1024-entry dedup (`markMessageSeen`) → maintenance gate → `loadConfigCached` → `messageAllowed` perms → short-circuits (`handleXFixup`, facebook-video / youtube-shorts) → `respondToMessage`: progress card + typing, both sent in the background and never awaited (`startRequestProgress` card loop posts the card, then `startTypingAfter` sends typing) → `prepareMessageResponse` → non-blocking `handoff` → `StreamChatCompletion` → live-edit render → `nodes.evictExcess()`. The card loop (`internal/app/progress.go`) owns the card message until the reply's first render, failure, or release calls `settleProgress`; until then it shows the web search queries and then "Reading N sources" (a count, never the source list; `showSearchStarted`/`showSearchFinished` from `runWebSearchToolPhase`) and stops refresh ticks once the reply is generating. TinyFish search fetches each result page in its own request under `tinyFishSearchFetchDeadline` (5 s); pages that miss it keep their snippet.
...
- Provider streaming: `ChatCompletionRouter.StreamChatCompletion` (`internal/providers/chat_client.go`) rotates `APIKeys` via `APIKeyRotator`, dispatches by `ProviderAPIKind` (OpenAI Chat-Completions SSE vs Responses API vs Gemini `GenerateContentStream`), streams `StreamDelta{Thinking, Content, FinishReason, ProviderResponseID, SearchMetadata, ToolCallResponse}`. Tool calling: one tool round per reply — the app appends the executed round as `ChatCompletionRequest.ToolRounds` (`ToolCallResponse` + one `FunctionToolOutput` per call), then forces the answer with `ToolChoice: ToolChoiceNone`; a backend that still returns tool calls (e.g. a proxy rewriting `none` to `auto`) gets `runToolFreeFinalAnswerRound` instead (no tools, round outputs appended to the latest user message as web search results) and is remembered per configured model (`toolChoiceNoneIgnored`), while OpenCode models (an `oc` name segment, `isOpenCodeModel`) go straight to it from their first reply; a round that offered no tools but still gets tool calls back (9router's decoy tools on OpenCode requests, `calledUnofferedTools`) is attempted again by `runGenerationRoundWithRetry`, up to `unofferedToolCallMaxAttempts` (3), before `errEmptyModelResponse`; providers replay rounds natively (`internal/providers/tool_rounds.go`: Chat Completions assistant `tool_calls` + `tool` messages; Responses output items + `function_call_output`, or `previous_response_id` of the tool-call response on chained turns). Retry: transient (EOF/5xx/429/reset) 5×/1s, queue-full 503 5×/3s, empty 5×; any `deltaReferencesContent` is final.
- Rendering: `responseTracker` + `segmentAccumulator` (`internal/app/response.go`) splits at 2000 runes, edits in place with ` ...` indicator; embed green/amber/red; `userFacingErrorMaxRunes=1500`. Discord has applied a streaming edit after the final one, so `respondToMessage` calls `confirmFinalRender` (`internal/app/final_render.go`): the tracker's `finalRender` (cleared by any later render) is re-read at +3 s and +15 s (`finalRenderCheckDelays`, set in `newBot`; test bots leave it empty) and restored when a message shows a streaming frame.
...
- State: `messageNodeStore` (mutex map, cap 500, per-node `mu`, `snapshotCache`, debounced `saveRequests` channel → SQLite, background hydrate); `configCache` stamped by mtime+size (`seedConfigCache`/`loadConfigCached`); `botState` (`RWMutex` + atomic generation + `saveMu`).
