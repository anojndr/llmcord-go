...
- **Auto-compaction** (`autoCompactRequest`, compaction.go): before streaming, the request is token-estimated with Codex's byte ratio (`ceil(bytes/4)`) against a limit mirroring Codex: `model_auto_compact_token_limit`, or `(context_window * 9) / 10` when unset (any configured value is clamped to that), with a hard cap at 95% of the window (`model_auto_compact_token_limit_scope` picks `total` vs `body_after_prefix`; `compact_prompt` overrides the summary prompt). If over, it summarizes the history with the model itself and replaces it with the newest user messages (≤20k tokens) plus the handoff summary appended last (universal prompts, not Codex's coding-flavored ones). Also derives the per-message text cap (`messageTextLimitForModel`, config.go:1135) and the context-window footer shown on replies.
